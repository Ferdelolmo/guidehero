...
import { randomString, THIRTY_MB_IN_BYTES, exportBundleIdToFile, ROLLUP_PLUGIN_NAME, consoleInfoOrange, shouldProcessFile, uploadCompressedSourceMaps, uploadSourceMap, faroBundleIdSnippet } from '@grafana/faro-bundlers-shared';
import fs from 'fs';
...
function faroUploader(pluginOptions) {
  ...
  const bundleId = (_a = pluginOptions.bundleId) !== null && _a !== void 0 ? _a : String(Date.now() + randomString(5));
  const uploadEndpoint = `${endpoint}/app/${appId}/sourcemaps/`;
  const maxSize = pluginOptions.maxUploadSize && pluginOptions.maxUploadSize > 0 ? pluginOptions.maxUploadSize : THIRTY_MB_IN_BYTES;
  ...
  return {
    ...
    async writeBundle(options, bundle) {
      ...
      if (verbose) {
        consoleInfoOrange(uploadedSourcemaps.length ? `Uploaded sourcemaps: ${uploadedSourcemaps.map(map => map.split('/').pop()).join(", ")}` : "No sourcemaps uploaded");
      }
    }
  };
}
...
export { faroUploader as default };
