...
function requireCommon () {
	...
	function setup(env) {
		...
		createDebug.humanize = require$$0$2;
...
function requireHelpers () {
	...
	function req(url, opts = {}) {
	    ...
	    const req = (href.startsWith('https:') ? https$1 : http$1).request(url, opts);
...
const uploadSourceMap = async options => {
  const {
    sourcemapEndpoint,
    filePath,
    apiKey,
  ...
  await fetch(sourcemapEndpoint, {
    ...
    // @ts-ignore
    agent: proxy ? new distExports.HttpsProxyAgent(proxy) : undefined
  }).then(res => {
    if (res.ok) {
      verbose && consoleInfoOrange(`Uploaded ${filename} to ${sourcemapEndpoint}`);
    } else {
      success = false;
      consoleInfoOrange(`Upload of ${filename} failed with status: ${res.status}`);
    }
    // delete source map
...
const uploadCompressedSourceMaps = async options => {
  const {
    sourcemapEndpoint,
    stackId,
    files,
  ...
  verbose && consoleInfoOrange(`Uploading ${files.map(file => file.split("/").pop()).join(", ")} to ${sourcemapEndpoint}`);
  await fetch(sourcemapEndpoint, {
    method: "POST",
    headers: {
    ...
    if (res.ok) {
      verbose && consoleInfoOrange(`Uploaded ${files.map(file => file.split("/").pop()).join(", ")} to ${sourcemapEndpoint}`);
    } else {
      success = false;
      consoleInfoOrange(`Upload of ${files.map(file => file.split("/").pop()).join(", ")} failed with status: ${res.status}`);
    }
    // delete tarball
    ...
    // delete source maps
    verbose && consoleInfoOrange(`Deleting ${files.map(file => file.split("/").pop()).join(", ")}`);
...
const shouldProcessFile = (filename, outputFiles) => {
  ...
  if (Array.isArray(outputFiles) && (outputFiles === null || outputFiles === void 0 ? void 0 : outputFiles.length)) {
    return includedInOutputFiles(filename, outputFiles);
  }
  return true;
};
const includedInOutputFiles = (filename, outputFiles) => {
...
const faroBundleIdSnippet = (bundleId, appName) => {
...
function randomString(length) {
  return crypto.randomBytes(length !== null && length !== void 0 ? length : 10).toString("hex");
}
const consoleInfoOrange = message => console.info(ansi.fg(214)`[Faro] ${message}`);
const WEBPACK_PLUGIN_NAME = "FaroSourceMapUploaderPlugin";
const ROLLUP_PLUGIN_NAME = "rollup-plugin-faro-source-map-uploader";
const ESBUILD_PLUGIN_NAME = "faro-esbuild-plugin";
const THIRTY_MB_IN_BYTES = 30 * 1024 * 1024;
 ...
 */
const exportBundleIdToFile = (bundleId, appName, verbose) => {
...
exports.ESBUILD_PLUGIN_NAME = ESBUILD_PLUGIN_NAME;
exports.JS_SOURCEMAP_PATTERN = JS_SOURCEMAP_PATTERN;
exports.ROLLUP_PLUGIN_NAME = ROLLUP_PLUGIN_NAME;
exports.THIRTY_MB_IN_BYTES = THIRTY_MB_IN_BYTES;
exports.WEBPACK_PLUGIN_NAME = WEBPACK_PLUGIN_NAME;
exports.cleanAppName = cleanAppName;
exports.consoleInfoOrange = consoleInfoOrange;
exports.exportBundleIdToFile = exportBundleIdToFile;
exports.faroBundleIdSnippet = faroBundleIdSnippet;
exports.randomString = randomString;
exports.shouldProcessFile = shouldProcessFile;
exports.uploadCompressedSourceMaps = uploadCompressedSourceMaps;
